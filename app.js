...
var productSrs = ['bag.jpg', 'banana.jpg', 'bathroom.jpg', 'boots.jpg', 'breakfast.jpg', 'bubblegum.jpg', 'chair.jpg', 'cthulhu.jpg', 'dog-duck.jpg', 'dragon.jpg', 'pen.jpg', 'pet-sweep.jpg', 'scissors.jpg', 'shark.jpg', 'sweep.png', 'tauntaun.jpg', 'unicorn.jpg',  'water-can.jpg', 'wine-glass.jpg'];
...
function Product(name) {                  // constructor
...
for (let i = 0; i < productSrs.length; i++) {      // object maker
    new Product(productSrs[i]);
}


function randomize() {                       // random number maker
...
function renderAlbum() {                        // photo screen
...
function clicks(event) {                                   // data screen
    if (round <= maxRounds) {
        let clickedImg = event.target.id;
        if (clickedImg === 'Left') {
            products[Left].votes++;
        }
        else if (clickedImg === 'Center') {
